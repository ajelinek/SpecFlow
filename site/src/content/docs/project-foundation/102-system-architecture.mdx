...
<Callout tone="optional" title="Optional but important">
  On projects where the technology choices are still being made, `102` is the right moment to make those decisions explicit and reviewable. Running feature design against an undocumented or misunderstood architecture is one of the more common sources of rework.
</Callout>
...
<PromptBlock title="Prompt" text={`Run 102-system-architecture. We are building a React SPA with a Node.js REST API, PostgreSQL, and deploying to AWS.`} />

<PromptBlock title="Prompt" text={`Run 102 for an Astro + SolidJS frontend with a Firebase backend.`} />
...
- If the project already has code, use `@explore` to discover what the actual stack is rather than assuming.
- Keep the testing strategy section concrete: which frameworks, where tests live, what the coverage expectations are.
- Reference D01 for user goals and business requirements — D02 should not re-state what D01 already captured.

## Common mistakes

- Listing technology names without recording the reasoning behind them
