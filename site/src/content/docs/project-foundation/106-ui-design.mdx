...
<PromptBlock title="Prompt" text={`Run 106 for a healthcare tool targeting clinical staff — professional, high-trust, desktop-first.`} />
...
- `106` calls `@designer` three times in parallel, each a fresh invocation with a different seed bet, to get genuinely divergent directions, then scores them itself in Step 4 — a single-option design process tends to produce mediocre output.
