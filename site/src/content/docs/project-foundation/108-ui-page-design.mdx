...
<PromptBlock title="Prompt" text={`Run 108 for the project dashboard page.`} />
...
- `108` calls `@designer` three times in parallel, each a fresh invocation with a different seed bet, to get genuinely divergent layouts, then scores them itself in Step 4 rather than committing to the first generated option.
