...
<Callout tone="core" title="301 is an orchestration workflow">
  `301` is not just a longer coding prompt. When the bundled workers are installed, coding runs go through `coder`, validation runs go through `validator`, and UI-affecting work should consult `designer` first. The phase structure keeps larger changes from collapsing into an undifferentiated coding session.
</Callout>
